...
EnergyCharts::EnergyCharts(wxPanel* parentpn, ceSerial* portCom)
    : wxPanel(parentpn, -1, wxPoint(-1, -1), wxSize(-1, -1))
{
    ...
    //Definition
    m_plot = new mpWindow(this, 5555, wxPoint(0, 0), wxSize(-1, -1));
...
void EnergyCharts::OnProgressTimer(wxTimerEvent& WXUNUSED(event)){
    ...
    if (com->IsOpened()) {
        ...
        m_plot->UpdateAll();
        //
        string string_name = to_string(vectorLayer->GetMaxY());
        Excel->writeEx(string_name);
    }
