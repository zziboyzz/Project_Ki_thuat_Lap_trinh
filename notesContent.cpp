...
END_EVENT_TABLE()
notesContent::notesContent(wxBoxSizer* parent, wxPanel* parentpn):
    wxPanel(parentpn, 333, wxPoint(-1, -1), wxSize(-1, -1))
{
    ...
    Tnote = new wxTextCtrl(LeftPanelCt, wxID_ANY, wxT("Ghi chu..."), wxPoint(0, 0), wxSize(0, 0), wxTE_MULTILINE);
    ...
    ReadEx = new CSVReader(fileName);
    // Get the data from CSV File
    std::vector<std::vector<std::string> > dataList = ReadEx->readEx();
